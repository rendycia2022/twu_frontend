...
const signIn = async () =>{
    ...
    if (sign?.value.email?.trim()) {
        ...
        const saving = await axiosSign.post('/auth/access', data);
        if(saving.data.status == 200){
            localStorage.setItem('token', saving.data.token); // save token
            router.push({ path: '/dashboard' });
            console.clear();
        }else{
            result(saving);
...
const result = (saving) =>{
    if(saving.status == 200){
        ...
        toast.add({ severity: 'Error', summary: saving.status, detail: saving.statusText, life: 4000 });
    }
}
            ...
            <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                    <div class="text-center mb-8">
                        ...
                        <div class="text-surface-900 dark:text-surface-0 text-3xl font-medium mb-4">Welcome to TWU System!</div>
                    ...
                    <div>
                        <div class="mb-8" >
                            ...
                            <InputText id="email" type="text" v-model.trim="sign.email" required="true" autofocus :invalid="submitted && !sign.email" placeholder="Email address" class="w-full md:w-[30rem]" />
                            <div>
                                <small v-if="submitted && !sign.email" class="text-red-500">Email is required.</small>
                            </div>
                        </div>
                        <div class="mb-4" >
                            <label for="password" class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2">Password</label>
                            <Password id="password" v-model="sign.password" :invalid="submitted && !sign.email" placeholder="Password" :toggleMask="true" class="" fluid :feedback="false"></Password>
                            <small v-if="submitted && !sign.password" class="text-red-500">Password is required.</small>
                        </div>
